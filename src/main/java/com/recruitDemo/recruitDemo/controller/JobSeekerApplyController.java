...
import com.recruitDemo.recruitDemo.services.*;
...
public class JobSeekerApplyController {
    ...
    @Autowired
    public JobSeekerApplyController(JobActivityService jobActivityService, UserService userService, JobSeekerApplyService jobSeekerApplyService, JobSeekerSaveService jobSeekerSaveService, RecruiterProfileService recruiterProfileService, JobSeekerProfileService jobSeekerProfileService) {
    ...
    public String display(@PathVariable("id") int id, Model model) {
        JobActivity jobDetails = jobActivityService.getOne(id);
        List<JobSeekerApply> jobSeekerApplyList = jobSeekerApplyService.getJobCandidates(jobDetails);
        List<JobSeekerSave> jobSeekerSaveList = jobSeekerSaveService.getJobCandidates(jobDetails);
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (!(authentication instanceof AnonymousAuthenticationToken)) {
            if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("Recruiter"))) {
                RecruiterProfile user = recruiterProfileService.getCurrentRecrutiterProfile();
                if (user != null) {
    ...
    public String apply(@PathVariable("id") int id, JobSeekerApply jobSeekerApply) {
        ...
        if (!(authentication instanceof AnonymousAuthenticationToken)) {
            ...
            if (seekerProfile.isPresent() && jobActivity != null) {
                ...
                jobSeekerApply.setJob(jobActivity );
